...
const p = path.join( // Dosya yolunu hesaplar ve p değişkenine atar.
  path.dirname(require.main.filename), // Ana uygulamanın çalıştırılabilir dosyasının dizinini alır.
  "data", // "data" dizini
  "cart.json" // "cart.json" dosyası
);

module.exports = class Cart { // Cart sınıfını oluşturur ve dışa aktarır.

  static addProduct(id, productPrice) { // Ürün eklemek için kullanılacak bir metot tanımlar ve iki parametre alır: id (ürün kimliği) ve productPrice (ürün fiyatı).

    fs.readFile(p, (err, fileContent) => { // "cart.json" dosyasını okur.
      ...
      const existingProductIndex = cart.products.findIndex(prod => prod.id === id); // Sepet içinde aynı ürünü arar ve indeksini alır.
      ...
      if (existingProduct) { // Eğer ürün sepette varsa:
      ...
      } else { // Eğer ürün daha önce eklenmemişse:
      ...
      fs.writeFile(p, JSON.stringify(cart), (err) => { // Güncellenmiş sepet bilgisini "cart.json" dosyasına yazdırır.
